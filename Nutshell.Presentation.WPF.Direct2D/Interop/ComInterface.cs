...
namespace Nutshell.Presentation.Direct2D1.Interop
{
        ...
        internal static class ComInterface
        {
                ...
                public delegate int CreateDeviceEx(
                        IDirect3D9Ex d3D9, uint adapter, int deviceType, IntPtr focusWindow, int behaviorFlags,
                ...
                [UnmanagedFunctionPointer(CallingConvention.StdCall)]
                public delegate int CreateTexture(
                        IDirect3DDevice9Ex device, uint Width, uint Height, uint Levels, int Usage, int Format, int Pool,
                        out IDirect3DTexture9 ppTexture, ref IntPtr pSharedHandle);

                [UnmanagedFunctionPointer(CallingConvention.StdCall)]
                public delegate int GetSurfaceLevel(
                        IDirect3DTexture9 texture, uint level, out IDirect3DSurface9 ppSurfaceLevel);
                ...
                [ComImport, Guid("0CFBAF3A-9FF6-429a-99B3-A2796AF8B89B"),
                 InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]
